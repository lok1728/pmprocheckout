...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<input type="hidden" id="level-initial_payment" name="level-initial_payment" value="<?php echo esc_attr($pmpro_level->initial_payment); ?>" />
	<?php if($pmpro_msg)
		{
	?>
	...
	<thead>
		<tr>
			<th>
				<?php
				$pmlevel = $pmpro_level->name;
				if ($pmlevel == "Donation with Free Membership") {
				$pmpagetitle = "Donation with Free Membership"; }
				else {$pmpagetitle ="Membership Type"; }
				?>
				<span class="pmpro_thead-name"><?php echo $pmpagetitle; ?></span>
				<?php if(count($pmpro_levels) > 1 && !$pmpro_review) { ?><span class="pmpro_thead-msg"><a href="<?php echo pmpro_url("levels"); ?>"><?php _e('change', 'pmpro');?></a></span><?php } ?>
				<?php
				$membershipname = $pmpro_level->name;
				if ($membershipname == "Donation with Free Membership" && !$pmpro_review) { ?><a href="../donate/" class="anonbutton">Donate Anonymously</a>
				<?php } else {} ?>
			</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>
					...
					if ($pmlevel != "Donation with Free Membership") { ?>
					<p>
					<?php printf(__('You have selected the <strong>%s</strong>.</br></br>', 'pmpro'), $pmpro_level->name);?>
					</p>
					<?php } else if(!$pmpro_review) { ?>
					<p><i>If you would prefer to make an anonymous donation and not receive the free membership,</i></p><p><i>please click on the "Donate Anonymously" button above.</i></p>
					<?php } ?>
				<?php
	...
	<?php if(pmpro_getGateway() == "paypal" && empty($pmpro_review)) { ?>
		...
		<thead>
			<tr>
				<th><h2><?php _e('Choose your Payment Method', 'pmpro');?></th>
			</tr>
		</thead>
		<tbody>
			<tr>
	...
	<thead>
		<tr>
			<th><?php _e('Contact Information', 'pmpro');?></th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>
				...
				<div>
					...
					<br />
					<div class="leftmar"><small class="lite">Please use hyphens, ex. 123-456-7890</small></div>
				</div>
...
<?php echo wpautop(do_shortcode($tospage->post_content));?>
	...
	<!-- Output TOTAL on checkout page -->
	<div class="ordertotalhead"><span class="pmpro_thead-name">Subtotal</span></div>
	<div class="membercost"><span>Membership Cost: </span><?php global $pmpro_currency_symbol; echo $pmpro_currency_symbol;?><span class="mcost"></span></div>
	<div class="optionaldonation"><span>Additional Contribution: </span><?php global $pmpro_currency_symbol; echo $pmpro_currency_symbol;?><span class="dcost"></span></div>
	<div class="ordertotaltext">TOTAL: <?php global $pmpro_currency_symbol; echo $pmpro_currency_symbol;?><span class="ordertotal"></span></div>
	
		<script>
		var total = 0;
		var total_donatevary = 0; // Variable Donation Membership
		var total_donation = parseFloat(jQuery('#donation_dropdown').val()) || 0; //optional donation
		var total_donatevary = parseFloat(jQuery('#price').val()) || 0; //variable donation
		if (total_donatevary > 0) {
			jQuery('.optionaldonation').hide(); // Hide the optional donation on the donate form
			var total_membercost = parseFloat(jQuery('#price').val()) || 0; // Get the variable input donation price set by the user and set it to the membership price
			jQuery('.membercost span').text('Donation Amount: '); // if this is only a donation level membership, replace Membership Level Text
			}
			else {
				var total_membercost = <?php echo esc_attr($pmpro_level->initial_payment); ?>;
			}
		
		var total = parseFloat(total_membercost + total_donation);
				var total = total.toFixed(2);
				var total_membercost = total_membercost.toFixed(2);
				var total_donation = total_donation.toFixed(2); // set decimal on donation cost
				jQuery('.ordertotal').text(total);
				jQuery('.mcost').text(total_membercost);
				jQuery('.dcost').text(total_donation);
		jQuery(document).ready(function()
		{
			function updateTotal() {
				...
				var total_donatevary = 0; // Variable Donation Membership
				var total_donation = parseFloat(jQuery('#donation_dropdown').val()) || 0; // optional donation
				//var total_cdcost = parseFloat(jQuery('#cdcost_total').val()) || 0;
				var total_donatevary = parseFloat(jQuery('#price').val()) || 0; //variable donation
				// Check for if on Donation Level
				if (total_donatevary > 0) {
					jQuery('.optionaldonation').hide(); // Hide the optional donation on the donate form
					var total_membercost = parseFloat(jQuery('#price').val()) || 0; // Get the variable input donation price set by the user and set it to the membership price
					jQuery('.membercost span').text('Donation Amount: '); // if this is only a donation level membership, replace Membership Level Text
						...
						var total_membercost = <?php echo esc_attr($pmpro_level->initial_payment); ?>; // normal membership level
				}
				var total = parseFloat(total_membercost + total_donation);
				var total = total.toFixed(2); //set decimal on total price
				var total_membercost = total_membercost.toFixed(2); //set decimal on member cost
				var total_donation = total_donation.toFixed(2); // set decimal on donation cost
				jQuery('.ordertotal').text(total);
				jQuery('.mcost').text(total_membercost);
				jQuery('.dcost').text(total_donation);
			}
			jQuery(document).bind('change keyup input', '#donation_dropdown', updateTotal);
		});
	</script>
	<!-- END TOTAL -->
	<div class="pmpro_submit">
		<?php if($pmpro_review) { ?>
			<?php //error_log("Session Return from Paypal: " . print_r($_SESSION, true), 3, "/home1/myiwbcor/tmp/pmprolog/pmpcheckouterror.log" ); ?>
			<script>
				// remove extra fields on review
				jQuery(document).ready(function() {
				var reviewing = jQuery('input[name="token"]').val();
					if (reviewing.length > 0) {
					jQuery('.pmpro_checkout-field').hide();
					}
				});
			</script>
...
<script>
	...
	jQuery('#pmpro_form').bind("keyup keypress", function(e) {
